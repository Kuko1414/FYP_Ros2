...
To achieve this powerful function, the design concept is as follows, ONLY WHEN IT IS PERMITTED TO PROCEED TO THE NEXT STEP CAN THE SUBSEQUENT FUNCTIONS BE WRITTEN.
  ...
  **Step 3.** AFTER ACHIEVING THE ABOVE FUNCTIONS, we added a depth camera. (Optimization: Distinguish between "detection distance" and "safe stopping distance". For instance, detect at 1.0m, and initiate braking so it safely stops at least 0.3m away). When an obstacle is detected in the path contour, a copy of the front image is saved to a dedicated `data/images` directory (not `/src/image` to prevent polluting source code). Based on the walkable area feedback from the depth camera, it plans a route to go around the obstacle. 
